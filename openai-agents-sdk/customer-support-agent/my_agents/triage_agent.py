"""Triage agent: the customer-facing entry point.

Responsibilities:
- INPUT guardrail: a dedicated agent classifies whether the request is on-topic
  (account / billing / orders / technical) before the triage agent runs.
- Dynamic, context-aware instructions (greets the customer by name, knows tier).
- HANDOFFS to the four specialist agents, carrying a structured HandoffData
  payload, logged via an on_handoff callback, with tool messages filtered out.
"""
...
def dynamic_triage_agent_instructions(
    ...
    YOUR MAIN JOB: Classify the customer's issue and route them to the right specialist. Only route to technical_agent for genuinely technical questions.
...
def make_handoff(agent):
    return handoff(
        ...
        # Strip prior tool messages from the conversation passed to the specialist.
