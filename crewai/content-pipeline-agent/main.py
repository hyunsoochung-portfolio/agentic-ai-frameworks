"""Content pipeline built with CrewAI Flows.

This is the "perfect main.py" from the third content-pipeline post. The Flow
orchestrates research -> content generation -> scoring -> (regenerate or
finalize) using @start/@listen/@router and the and_/or_ combinators.

It generates a tweet, a blog post, or a LinkedIn post depending on
`content_type`, then scores it (SEO for blogs, virality for tweet/linkedin) via
two helper Crews (SeoCrew, ViralityCrew). If the score is below threshold the
content is regenerated with the score's reason as feedback.
"""
...
class ContentPipelineFlow(Flow[ContentPipelineState]):
    ...
    def conduct_research(self):
        researcher = Agent(
            ...
            backstory=(
                "You're like a digital detective who loves digging up "
                "fascinating facts and insights. You have a knack for finding "
                "the good stuff that others miss."
            ),
