"""Pydantic models that enforce a specific structured output from the LLM.

Each model maps to a sequential stage of the job-analysis workflow and is wired
into the corresponding Task via `output_pydantic=...`, which forces the LLM to
emit a valid JSON object that matches the schema.
"""
...
class Job(BaseModel):
    ...
    company_name: str
    job_location: str
    is_remote_friendly: bool | None = None  # default = None
